...
AllUsersTopCountries = AllUsers['country'].value_counts()
AllUsersTopCountries = AllUsersTopCountries.reset_index()
AllUsersTopCountries.columns=['country','count']

top25CountriesList = AllUsersTopCountries[:25]['country'].tolist()
...
forbiddenArtistList = []
forbiddenArtistList = forbiddenArtistList + top5000artistslist

counter = 0
for country in top25CountriesList:
    cGroup = AllUsers.loc[(AllUsers['country'] == country)]
    TopCArtists = cGroup['artist_id'].value_counts()
    TopCArtists = TopCArtists.reset_index()
    TopCArtists.columns = ['artist_id', 'counts']
    TopCArtists = TopCArtists[~TopCArtists['artist_id'].isin(forbiddenArtistList)]
    filename = country + "Top200Artists.csv"
    TopCArtists[:200].to_csv(filename, sep="\t", index = False)
    forbiddenArtistList = forbiddenArtistList + TopCArtists[:200]['artist_id'].tolist()
